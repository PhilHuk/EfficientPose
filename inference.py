...
def main():
    ...
    #input parameter
    path_to_images = "/home/phil/Code/atp_dataset_5/pictures"
    image_extension = ".png"
    phi = 0
    path_to_weights = "/home/phil/Code/EfficientPose/checkpoints/weights_412.h5"
    save_path = "/home/phil/Code/atp_dataset_5/pictures/predictions_412/" #where to save the images or None if the images should be displayed and not saved
    # save_path = None
    # class_to_name = {0: "ape", 1: "can", 2: "cat", 3: "driller", 4: "duck", 5: "eggbox", 6: "glue", 7: "holepuncher"} #Occlusion
    class_to_name = {0: "pyramide", 1: "platte", 2: "wuerfel", 3: "adapter", 4: "kugel"}
    score_threshold = 0.7
    ...
    for image_filename in tqdm(image_list):
        ...
        if save_path is None:
            ...
            cv2.imwrite(os.path.join(save_path, image_filename.replace(image_extension, "_predicted_calibrated" + image_extension)), original_image)
...
def get_linemod_camera_matrix():
    ...
    """
    #return np.array([[572.4114, 0., 325.2611], [0., 573.57043, 242.04899], [0., 0., 1.]], dtype = np.float32)  # Linemod
    return np.array([[256, 0., 256], [0., 256, 256], [0., 0., 1.]], dtype = np.float32)       # NDDS
    #return np.array([[821.993, 0., 330.489], [0., 821.993, 248.997], [0., 0., 1.]], dtype = np.float32)       # Logitech C310
    #return np.array([[180, 0., 50], [0., 180, 130], [0., 0., 1.]], dtype = np.float32)
...
def get_linemod_3d_bboxes():
    ...
    """
    name_to_model_info = {  'platte':       {'diameter': 169.0739, 'min_x': -67.5, 'min_y': -50.0, 'min_z': -1.0, 'size_x': 135.0, 'size_y': 100.0, 'size_z': 19.0},
                            'pyramide':     {'diameter': 106.972, 'min_x': -1.0, 'min_y': -1.0, 'min_z': -39.5, 'size_x': 51.0, 'size_y': 51.0, 'size_z': 79.0},
                            'wuerfel':      {'diameter': 103.6388, 'min_x': -39.5, 'min_y': -30.0, 'min_z': -15.0, 'size_x': 79.0, 'size_y': 60.0, 'size_z': 30.0},
                            'adapter':      {'diameter': 26.5579, 'min_x': -7.3, 'min_y': -4.8, 'min_z': -10.0, 'size_x': 14.6, 'size_y': 9.6, 'size_z': 20.0},
                            'kugel':        {'diameter': 93.3833, 'min_x': -30.0, 'min_y': 0.0, 'min_z': -30.0, 'size_x': 60.0, 'size_y': 38.9928, 'size_z': 60.0},
                            "ape":          {"diameter": 102.09865663, "min_x": -37.93430000, "min_y": -38.79960000, "min_z": -45.88450000, "size_x": 75.86860000, "size_y": 77.59920000, "size_z": 91.76900000},
